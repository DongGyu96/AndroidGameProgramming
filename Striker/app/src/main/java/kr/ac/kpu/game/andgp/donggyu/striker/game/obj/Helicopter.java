        ...
        if(y > 0.f) {
            ...
            if(attackCoolTime < 0.f) {
                ...
                for (GameObject obj : player) {
                    ...
                    SecondScene.get().getGameWorld().add(SecondScene.Layer.enemy_bullet.ordinal(), Bullet.get(x, y, 30, 30, R.mipmap.enemy_bullet, xDir * 400.f, yDir * 400.f, false));
                }
                attackCoolTime = MAX_ATTACK_COOLTIME;
            }
        }

        else if(x < -50.f || x > UIBridge.metrics.size.x + 50.f || y > UIBridge.metrics.size.y) {
